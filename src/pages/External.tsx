...
export default function ContestPage() {
  ...
  // 마운트 시 대외활동(EXTERNAL) 목록 로드
  useEffect(() => {
    const controller = new AbortController()
    setLoading(true)
    setError(null)

    searchAllEvents({ category: 'EXTERNAL' }, controller.signal)
      .then(setEvents)
      .catch((err: unknown) => {
        if (err instanceof DOMException && err.name === 'AbortError') return
        setError(err instanceof Error ? err.message : '대외활동 목록을 불러오지 못했어요.')
      })
  ...
  const filtered = useMemo(() => {
    ...
    list = list.filter((e) => !dDayInfo(e.endDate).closed) // 마감된 대외활동 제외
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 pb-10 pt-4">
      ...
      <header className="rise-in mb-8">
        <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-[#1B1B33]">대외활동</h1>
        <p className="mt-1.5 text-sm text-[#6F7095]">전국의 다양한 대외활동을 한눈에 확인하고 도전해보세요!</p>
      </header>
