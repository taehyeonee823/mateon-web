...
export async function getRecommendedUsers(params: GetRecommendedUsersParams) {
  ...
  try {
    ...
    if (isApiErrorPayload(e)) {
      ...
      if (status === 400 && message.includes('TEAM_EMBEDDING_NOT_READY')) {
        throw new TeamEmbeddingNotReadyError(message);
      }
      if (status === 400 && message.includes('MATCHING_INTENT_REQUIRED')) {
        throw new MatchingIntentRequiredError(message);
      }
      if (status === 400 && message.includes('FORBIDDEN_ACCESS')) {
        throw new ForbiddenAccessError(message);
      }
      if (status === 400 && message.includes('RESOURCE_NOT_FOUND')) {
        throw new ResourceNotFoundError(message);
      }
      throw new Error(message);
    }
    throw e; // 공통 에러(403/500/502/503)는 apiErrorHandler에서 이미 던져진 상태
  }
}
...
export async function getUserRecommendationReason(
  ...
  try {
    ...
    if (isApiErrorPayload(e)) {
      ...
      if (status === 404 && message.includes('RECOMMENDATION_NOT_FOUND')) {
